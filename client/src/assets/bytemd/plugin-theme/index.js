...
// 引入 require.context 用于动态加载模块
const context = require.context('/node_modules/highlight.js/styles', false, /\.min\.css$/)
const highlightThemes = {}

// 只调用一次 context.keys() 来获取所有键
const keys = context.keys()

keys.forEach((key) => {
  // 使用正则表达式清理键，删除 './' 和 '.min.css'
  const name = key.replace(/(^\.\/)|\.min\.css$/gi, '').replace('/node_modules/highlight.js/styles/', '')

  // 调用 context 作为一个函数来加载模块
  const theme = context.key

  // 将主题添加到 highlightThemes 对象中
