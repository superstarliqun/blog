    ...
    <el-dialog :title="dialogTitle" :visible="true" width="1200px" :before-close="handleClose">
      <el-form ref="volunteerForm" :model="volunteerForm" label-width="90px" :rules="rules">
        <div class="basic_choose">
          ...
          <p class="info">
            年份：<span>{{ paramObj.yearText }}</span>
          </p>
        </div>
        <div class="container">
          <div v-for="(item, index) in batchListArr" :key="index" class="moudle_container">
            <el-form :ref="'ruleFormRef' + index" :model="ruleForm[index]" label-width="90px" :rules="rules">
              <p class="batch-type">批次：<span>{{ item.name }}</span></p>
              <div class="moudle_volunteer">
                <!-- 删除按钮 -->
                <div class="close-icon" @click="handleDelete(i)">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        sysProvinceId: [{ required: true, message: '请选择省份', trigger: 'blur' }],
        voluntNum: [
          { required: true, message: '请填写数字', trigger: 'blur' },
          { pattern: /^[1-9]\d*$/, message: '请输入非0的正整数', trigger: 'blur' }
        ],
        voluntGroupSpecialityNum: [{ required: true, message: '请填写数字', trigger: 'blur' }]
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    getBatchList() {
      ...
      if (res.code === 200) {
        ...
        this.ruleForm = this.batchListArr
      } else {
        this.$message.error(res.msg)
      }
      // })
    ...
    queryProvince() {
      ...
      if (res.code === 200) {
        ...
        if (this.paramObj.provinceId !== '0' && this.paramObj.tipSuccess !== 2) {
    ...
    handleAddForm() {
      ...
      const isVoluntNum = dataArr.every(item => item.voluntNum)
      const isSpecialtyNum = dataArr.every(item => item.voluntGroupSpecialityNum)
    ...
    handleEditingForm() {
      ...
      const params = { ...this.volunteerForm }
      const pattern = /^\d+$/
      console.log(typeof params.voluntGroupSpecialityNum, '编辑')
      if (params.voluntNum === '' || params.voluntGroupSpecialityNum === '' || !pattern.test(params.voluntNum) || !pattern.test(params.voluntGroupSpecialityNum)) {
        this.$message.error('志愿数量不能为空并且只能输入数值')
        return false
      } else {
        this.reqM1Service(url, params).then(res => {
    ...
    data1() {
      return { 'code': 200, 'msg': '成功', 'data': [{ 'id': 1, 'name': '北京市', 'sysCityList': null }, { 'id': 2, 'name': '上海市', 'sysCityList': null }, { 'id': 3, 'name': '天津市', 'sysCityList': null }, { 'id': 4, 'name': '重庆市', 'sysCityList': null }, { 'id': 5, 'name': '香港特别行政区', 'sysCityList': null }, { 'id': 6, 'name': '澳门特别行政区', 'sysCityList': null }, { 'id': 7, 'name': '黑龙江省', 'sysCityList': null }, { 'id': 8, 'name': '吉林省', 'sysCityList': null }, { 'id': 9, 'name': '辽宁省', 'sysCityList': null }, { 'id': 10, 'name': '河北省', 'sysCityList': null }, { 'id': 11, 'name': '陕西省', 'sysCityList': null }, { 'id': 12, 'name': '河南省', 'sysCityList': null }, { 'id': 13, 'name': '山东省', 'sysCityList': null }, { 'id': 14, 'name': '山西省', 'sysCityList': null }, { 'id': 15, 'name': '青海省', 'sysCityList': null }, { 'id': 16, 'name': '甘肃省', 'sysCityList': null }, { 'id': 17, 'name': '安徽省', 'sysCityList': null }, { 'id': 18, 'name': '湖北省', 'sysCityList': null }, { 'id': 19, 'name': '湖南省', 'sysCityList': null }, { 'id': 20, 'name': '江苏省', 'sysCityList': null }, { 'id': 21, 'name': '四川省', 'sysCityList': null }, { 'id': 22, 'name': '贵州省', 'sysCityList': null }, { 'id': 23, 'name': '云南省', 'sysCityList': null }, { 'id': 24, 'name': '浙江省', 'sysCityList': null }, { 'id': 25, 'name': '江西省', 'sysCityList': null }, { 'id': 26, 'name': '广东省', 'sysCityList': null }, { 'id': 27, 'name': '福建省', 'sysCityList': null }, { 'id': 28, 'name': '台湾省', 'sysCityList': null }, { 'id': 29, 'name': '海南省', 'sysCityList': null }, { 'id': 30, 'name': '内蒙古自治区', 'sysCityList': null }, { 'id': 31, 'name': '广西壮族自治区', 'sysCityList': null }, { 'id': 32, 'name': '西藏自治区', 'sysCityList': null }, { 'id': 33, 'name': '宁夏回族自治区', 'sysCityList': null }, { 'id': 34, 'name': '新疆维吾尔自治区', 'sysCityList': null }], 'rootToken': null, 'uidtoken': null, 'rootSessionId': null }
    },
    data2() {
      return { 'code': 200, 'msg': '成功', 'data': [{ 'id': 85, 'sysProvinceId': 2, 'time': 2023, 'name': '本科批', 'batchZsMateName': '', 'batchSocreMatchName': '', 'sysProBatchLine': null, 'examType': null, 'batchScore': null, 'specialityLevelId': 1, 'specialityLevelName': '本科' }], 'rootToken': null, 'uidtoken': null, 'rootSessionId': null }
    }
  }
}
</script>
